...
def train_pareto_mtl(objectives, num_epochs=1000):
    model = nn.Linear(1, 1)
    ...
    for _ in range(num_epochs):
        ...
        loss = torch.norm(obj_values, p=2)
        loss.backward()
        optimizer.step()
...
pareto_points = train_pareto_mtl(objectives)
plt.scatter([p[0] for p in pareto_points], [p[1] for p in pareto_points])
plt.xlabel('Objective 1')
plt.ylabel('Objective 2')
plt.title('Pareto Front (Pareto MTL)')
plt.show()
