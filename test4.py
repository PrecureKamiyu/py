...
possible_workloads = [0 for _ in possible_locations]

distances = [0 for _ in range(len(possible_locations))]
...
for i in range(len(possible_locations)):
    ...
    Kth_nearest_distance = sorted_distances[:number_of_cluster+1][-1]
    print(Kth_nearest_distance)
    distances[i] = sorted_distances[:number_of_cluster+1][-1][0]


balances = [0 for _ in range(number_of_possible_locations)]
